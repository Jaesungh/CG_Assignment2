...
vec3 phongShading(const vec3& point, const vec3& normal, const vec3& viewDir, const vec3& lightDir, const vec3& lightColor, const vec3& ka, const vec3& kd, const vec3& ks, int specularPower) {
	...
	float spec = pow(glm::max(dot(viewDir, reflectDir), 0.0f), specularPower);
		...
		for (int i = 0; i < Width; ++i)
		{
			...
			if (scene.intersect(ray, t, ka, kd, ks, specularPower, normal)) {
				...
				if (!isInShadow(point, lightDir)) {
					color = phongShading(point, normal, viewDir, lightDir, light.color, ka, kd, ks, specularPower);
				}
				else {
					color = ka * light.color; // Only ambient component if in shadow
				}
